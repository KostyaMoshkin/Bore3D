...
namespace GL {
	...
	void SceneRender::setViewAngle(float fZoom_)
	{
		...
		Matrix4 mPerspective = glm::perspective(glm::radians(m_fViewAngle + fZoom_), float(m_nScreenWidth) / m_nScreenHeight, 0.1f, 50.0f);
	...
	{
		Matrix4 mTranslate = Matrix4(1.0f);
		mTranslate = glm::translate(mTranslate, Vector3(fMoveV_, 0, 0));
		mTranslate = glm::translate(mTranslate, Vector3(0, fMoveY_, 0));
	...
	//{
	//	Matrix4 mView = glm::lookAt(fCamPosition_, glm::normalize(fCamPosition_ - vCamCenter_), glm::normalize(vCamUp_));  //  eye, center, up
